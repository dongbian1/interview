...
/**
  ...
  const moduleRendering = (item: ModalFormItem) => {
    switch (item.type) {
      case 'input':
        return <Input placeholder={`请输入${item.title}`} disabled={type === 'update' && item.disabled} />
      case 'password':
        return <Input.Password placeholder={`请输入${item.title}`} disabled={type === 'update' && item.disabled} />
      case 'inputNumber':
        return (
          <InputNumber
            ...
            style={{ width: '100%' }}
            min={0}
            disabled={type === 'update' && item.disabled}
          />
        )
      case 'textArea':
        return <Input.TextArea rows={2} disabled={type === 'update' && item.disabled} />
      case 'select':
        return (
          <Select
            showSearch
      ...
      case 'upload':
        return (
          ...
          >
            {fileList && fileList.length >= item.uploadNumber! ? null : uploadButton}
          </Upload>
        )
      case 'datePicker':
...
return (
    <Modal
      ...
      onOk={onFormSubmit}
    >
      <Form {...Layout()} form={usForm} name="register" initialValues={data} scrollToFirstError>
        <Row>
          {formItem.map((item, index) => {
            return (
              <Col key={index} span={typeSetting === 'one' ? 24 : 12}>
                <Form.Item
                  name={item.name}
                  label={item.title}
                  rules={item.rules}
...
import { Col, DatePicker, Form, Input, InputNumber, Modal, Radio, Row, Select, Switch, Upload, TreeSelect } from 'antd'
import { FormInstance } from 'antd/lib/form'
...
export const OperateModal: React.FC<IOperateModal> = ({
  ...
  const onFormSubmit = () => {
    usForm.validateFields().then(values => {
      onSubmit({
        ...data,
        ...values,
        doorPhoto: fileList.length > 0 ? fileList : [],
      })
    })
  }
  ...
  const handleChange = (file: any) => {
    const newFileList = file.fileList.map((fileItem: any, index: number) => {
